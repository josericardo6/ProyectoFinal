...
public class Cola <T extends Comparable<T>> {
     ...
     * <h1>ToString</h1>
     * <p>
     * Imprime las personas de la cola.</p>
     *
     * @return String: Lista.
     */
    @Override
     ...
     */
    public boolean queue(String nombre, String apellido, String apellido2, String documento, double edad) {
     ...
     * <h1>MergeSort</h1>
     * <p>
     * Ordena la cola utilizando el metodo de ordenamiento por mezcla</p>
     *
     * @param inicial Nodo donde empieza la lista.
     * @return Nodo: Cabeza.
     */
    public Nodo mergeSort(Nodo inicial) {
        //Divide la cola esta que sea nula o solo tenga 1 elemento en la lista.
        if (inicial == null || inicial.getSiguiente() == null) {
            return inicial;
        }
        
        //Divide la cola en 2 listas.
        //Encuentra el nodo de la mitad de la cola y divide la cola en 2.
        Nodo mitad = dividirCola(inicial);
        Nodo sigMitad = mitad.getSiguiente();
        mitad.setSiguiente(null);

        //Divide la cola hasta que tenga solo un elemento cada uno.
     ...
     */
    public void mergeCola(Nodo izqIni, Nodo derIni) {
        Nodo aux;
        Nodo mezclar = null;
     ...
     * @return Nodo: Puntero.
     */
    public Nodo dividirCola(Nodo inicial) {
        if (inicial == null) {
            return inicial;
        }
        Nodo aux1 = inicial;
     ...
     * <p>
     * Busca si hay pasaportes en la cola de ser asi los agrega de primero.</p>
     */
    public void pasaportePrimero() {
        Nodo aux = cabeza;
        Cola cedulaAux = new Cola();
        Cola pasaporteAux = new Cola();

        while (aux != null) {
            if (pasaporteCedula(aux.getPersona().getCedula())) { //Si es un pasaporte lo agrega a la lista auxiliar de pasaportes.
                pasaporteAux.queue(aux.getPersona().getNombre(), aux.getPersona().getApellido(),
                        aux.getPersona().getApellido2(), aux.getPersona().getCedula(), aux.getPersona().getEdad());
            } else {
                cedulaAux.queue(aux.getPersona().getNombre(), aux.getPersona().getApellido(), //Sino lo agrega a la lista auxiliar de cedulas.
                        aux.getPersona().getApellido2(), aux.getPersona().getCedula(), aux.getPersona().getEdad());
            }
            aux = aux.getSiguiente();
        }
        ...
        agregarDesdeColaAux(pasaporteAux);
        agregarDesdeColaAux(cedulaAux);
    }
    
    /**
     * <h1>AgregarDesdeColaAux</h1>
     * <p>
     * Agrega a las persona desde otras colas. Y vacia la cola auxiliar.</p>
     *
     * @param colaAux
     */
    public void agregarDesdeColaAux(Cola colaAux) {
        if (!colaAux.isEmpty()) {
            Nodo aux = colaAux.cabeza;
            while (aux != null) {
                queue(aux.getPersona().getNombre(), aux.getPersona().getApellido(), //Agrega los pasaportes a la Lista original.
                        aux.getPersona().getApellido2(), aux.getPersona().getCedula(), aux.getPersona().getEdad());
                aux = aux.getSiguiente();
            }
            int cont = colaAux.size;
            while (cont > 0) {
                colaAux.dequeue(0);
                cont--;
            }
